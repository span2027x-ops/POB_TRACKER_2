...
# Simple shared PIN for Admin / Manager Mode (change this to whatever you like)
ADMIN_PIN = "1234"
...
def format_booking_label(index, row):
    """Human-readable label for a booking row, used in dropdowns for safe selection."""
    return f"#{index} | {row['Date']} | {row['MR']} | {row['Chemist']} | {row['Product']} | {row['Units']} units | {row['Supply Status']}"
